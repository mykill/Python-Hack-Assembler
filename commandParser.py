...
# c) Identify and obtain Label (ROM) symbols, and add them to symbol_Table. This happens here because
...
def commandType(lines, symbol_Table):
	commands = []
...
def compute(commands): #, hack_file, symbol_Table, var_sym_start):
        for command in commands:
                if command.type == 'C_Command':
                        if '=' in command.cmd_string: # If there is an equal sign in the command
                                ...
                                cmp = compmand.cmd_string.partition(';')[0]
        print(Code.comp(cmp),'comp')
        return (Code.comp(cmp))

def destination(commands):#, hack_file, symbol_Table, var_sym_start):
        for command in commands:
                if command.type == 'C_Command':
                        if '=' in command.cmd_string:
                                dest = command.cmd_string.partition('=')[0] # Get the dest bits
                        elif ';' in command.cmd_string: # Same thing if there is an ';' in the command, except the dest bits are '000'
                                dest = None
        print(Code.dest(dest),'dest')
        return (Code.dest(dest))
			
def jump(commands):#, hack_file, symbol_Table, var_sym_start):
        for command in commands: # For all commands	
                if command.type == 'C_Command': # In the case of a C_Command
                        if '=' in command.cmd_string: # If there is an equal sign in the command
...
# have been identified and added to symbol_Table in the previous loop (make_commands), we can now add all RAM
...
def Acommands(commands, symbol_Table, var_sym_start):
	for command in commands: # For all commands
                if command.type == 'A_Command': # In the case of A_Commands
                        label = command.cmd_string # ...grab the A_Command itself
                        if (not label.isdigit()) and (label not in symbol_Table.keys()): # Check if the A_Command is a variable label, and not already entered into symbol_Table
                                symbol_Table[label] = var_sym_start # Enter new Variable Label into Symbol Table
                                var_sym_start = var_sym_start + 1 # Set memory location of next Variable Symbol
                        if command.cmd_string in symbol_Table.keys(): # If the A_Command is a Variable Symbol for a RAM address
                                address = int(symbol_Table[command.cmd_string]) # Grab the corresponding decimal address
                        else:
	...
	return address
			#hack_file.write(address + '\n') # Write the address specified by the A_Command to the .hack file
                        ...
                        #hack_file.write('111' + compute(commands)+ '\n')
			#hack_file.write('111' +' cmp ' + compute(commands) + ' dest '+ destination(commands)+' jump ' + jump(commands) + '\n') # Write the C_Command to the .hack file.

def writeCommands(commands, hack_file, symbol_Table, var_sym_start):
        for command in commands: # For all commands
                #print(Acommands(commands, symbol_Table, var_sym_start) + '\n') # Write the address specified by the A_Command to the .hack file
                print('111' + compute(commands) + destination(commands) + jump(commands) + '\n')
